...
四张人格卡并排（大屏一行四张，中屏两列，手机两列紧凑卡），参考 8bitcn 的 `Duel Block` / `Save Slots` 区块样式：
...
- **桌面断点**：宽 ≥768px 且高 ≥501px 时保持现有横排（棋盘 22px 格子 + 180×150 立绘）。鼠标指针不出现虚拟方向键；平板等粗指针设备仍显示十字键，不改桌面构图。
- **竖屏（窄或矮）**：HUD 贴顶；棋盘按槽位取整铺满屏宽（正方形，比例不变）。暂停在棋盘右下角外侧。棋盘下方左侧约 128px 头像，右侧通栏台词气泡（尖角朝左指向头像）；底栏右侧为大号十字键。
- **横屏（窄或矮）**：棋盘按左槽 `min(宽,高)` 取最大正方形，并在左槽内居中（视觉上靠屏幕中间，右侧让给操作区）。暂停在最右上角。气泡与约 96px 头像叠在一起，贴在十字键左侧（气泡在上头像在下，与方向键底对齐）；无台词时不留气泡空位。
- **虚拟十字键**：仅 `(pointer: coarse)` 显示。compact 用大号键（约 56px），走文档流不叠在立绘上。`pointerdown` 写入按住方向，松手清空；与键盘共用同一套方向队列，不改 tick。不实现滑动手势。暂停/结算弹层 z-index 高于方向键。
...
- 面板固定在棋盘一侧边缘（桌面端），使用该 AI 的主题色作为边框；竖屏棋盘下方左侧放大头像、右侧通栏气泡（尖角朝左）；横屏暂停在最右上角，气泡与头像贴在十字键左侧（底对齐），尖角指向头像；
...
| 虚拟十字方向键 | 自定义 `VirtualDPad`：3×3 像素十字，仅粗指针设备显示 |
...
- 滑动手势操作（与虚拟十字键抢输入，不做）；
- AI vs AI 观战、排行榜等页面（V1 范围之外的功能，本文档不涉及）。
