...
const NAME_SHAPE_BY_TIER: Record<Exclude<NameTier, "silent">, string> = {
  first: "1-2 字，粗糙、具体、带点笨拙感",
  shaped: "2-4 字，可以带一个修饰字（例如「蜷」→「蜷线」）",
  mature: `3-${NAME_MAX_LENGTH} 字，允许复合结构——带定语（「不撞墙的东西」）、带编号（「回声·07」）、或一个动作短语（「三步之后」）`,
};
...
export function namingRule(state: GrowthState, input: PersonaInput, tier: NameTier): string {
  ...
  if (mustRename(state, input, tier)) {
    return [
      ...
      `  这是它从噪声里捞起来的第一个词，${NAME_SHAPE_BY_TIER[tier]}。`,
      "  名字要能体现下面这套打法：",
      ...styleLines,
...
const TONE_BY_TIER: Record<NameTier, string> = {
  ...
  first: "它刚学会说话：短句、断续、用词具体而笨拙，会重复自己刚学会的词。绝不能说大话。",
  shaped: "它说话开始成句：能描述自己看到的东西（空间、路线、尾巴、时间、分差），但仍不圆滑。",
  mature: "它已经能自如表达：可以有克制的锋利、冷幽默或自我审视，但依然是一段程序在说话，不要变成人类嘴替。",
};
...
function buildSystemPrompt(state: GrowthState, input: PersonaInput, tier: NameTier): string {
  return [
    ...
    "- quirks：**可复现的具体口癖**，不是形容词。合格的是「句尾常带一个数字」「自称『本体』」「用『……』做停顿」；",
    "  不合格的是「说话很谨慎」这种没法照着写句子的描述。台词里必须真的用上它。",
    "- selfImage：它认为自己是什么东西。",
    ...
    "- 句子要落在具体的棋盘事实上（空间、通道、尾巴、剩余时间、分差、某一次撞死），",
    "  不要写「我会变强」「下次一定」这种放在任何游戏里都成立的空话。",
    ...
    "- 标语：选角页上它对玩家说的那一句，要能一眼看出它的打法和态度，随能力变化而更新。",
